...
if(!empty($settings['disable_all_crons']) || !empty($settings['disable_insights_cron']) || !empty($settings['enable_maintenance_mode'])){
	exit(0);
...
/**
 * Reset schedule group status
 */
reset_schedule_group_status();

/**
 * select posts to scrape insights
 */

/**
 * disable next insights for posts older than 7 days
 */
sql_query("UPDATE post_log SET post_log.next_insight = '0000-00-00 00:00:00' WHERE post_log.posted_at < DATE_SUB(NOW(), INTERVAL 7 DAY)");
...
$q = sql_query("SELECT post_log_id FROM post_log 
				  ...
				  WHERE 
					  ...
					  post_log.next_insight != '0000-00-00 00:00:00' AND
					  post_log.next_insight <= NOW() AND
...
while($res = sql_fetch_assoc($q)){
	...
	$post[] = array('api_key' => API_KEY, 'cmd' => 'scrape_insights', 'log_id' => $res['post_log_id']);
